...
def nim1():
    print(
        'Правила такие: Мы по очереди берем камни из кучки. Можно взять от 1 до 3 камней. Выигрывает тот, кто возьмет '
        'последний камень'
    )
    while True:
        print()
        stones_total = int(input(
            'Можешь выбрать количество камней в кучке: '
        ))
        ...
        while stones_total > 0:
            if (-1) ** count > 0:
                ...
                print('Ваш ход')
                stones = int(input(
                    'Сколько камней ты берешь? '
                ))
                while stones < 1 or stones > 3 or stones % 1 != 0:
                    stones = int(input(
                        'Количество камней должно быть целым числом от 1 до 3 включительно. Сколько камней ты '
                        'берешь? '
                    ))
                count += 1
            stones_total -= stones
            print('Взяли: ', stones, '. Осталось: ', stones_total, '|', '*' * stones_total, '|')
        if (-1) ** count < 0:
            b = input('Ты проиграл. Играем снова: да или нет? ')
            if b == 'нет':
                print('Сфинкс съел ваш мозг. Конец.')
                return 0
            elif b == 'да':
...
def stairs_down_room6(local_from_room):
    global game_nim1
    eligible_rooms = [5, 7]
    if local_from_room not in eligible_rooms:
        print(
        ...
        call_error_handler()
    elif local_from_room == 5 and game_nim1 == 0:
        print(
            'Вы долго спускались по лестнице. И, наконец, оказались перед дверью с головой сфинкса. Глаза сфинкса '
            'открылись и он сказал:'
            '\n- Чтобы пройти в эту дверь, ты должен выиграть у меня в игру!'
        )
        game_nim1 = nim1()
        if game_nim1 == 0:
            return death(local_from_room, 6)
        return 6, 7
    elif local_from_room == 5 and game_nim1 == 1:
        print(
            'Вы долго спускались по лестнице, прошли открытые двери с головой сфинкса - и вы вошли в узкий коридор с '
            'низким потолком.'
        )
        return 6, 7
    elif local_from_room == 7:
        print('В разработке')


from_room = 0  # начинаем сначала
current_room = 1
game_nim1 = 0  # есть ли победа в ним1
...
while True:
    ...
    elif current_room == 5:  # лестница верх
        from_room, current_room = stairs_up_room5(from_room)
    elif current_room == 6:  # лестница низ
        from_room, current_room = stairs_down_room6(from_room)
